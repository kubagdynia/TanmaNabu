...
namespace Entitas
{
    ...
    public class Context<TEntity> : IContext<TEntity> where TEntity : class, IEntity
    {
        ...
        {
            List<IGroup<TEntity>> groups = _groupsForIndex[index];
            if (groups == null) return;

            List<GroupChanged<TEntity>> events = _groupChangedListPool.Get();

            TEntity tEntity = (TEntity)entity;

            foreach (IGroup<TEntity> group in groups)
            {
            ...
            {
                GroupChanged<TEntity> groupChangedEvent = events[i];
        ...
        private void OnDestroyEntity(IEntity entity)
        {
            ...
            bool removed = _entities.Remove(tEntity);
            if (!removed)
            {
