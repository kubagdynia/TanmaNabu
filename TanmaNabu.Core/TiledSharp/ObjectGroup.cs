...
namespace TiledSharp
{
    public class TmxObjectGroup : ITmxLayer
    {
        ...
        public TmxObjectGroup(XElement xObjectGroup)
        {
            ...
            Dictionary<string, DrawOrderType> drawOrderDict = new Dictionary<string, DrawOrderType>
            {
            ...
            string drawOrderValue = (string)xObjectGroup.Attribute("draworder");
            ...
            foreach (XElement element in xObjectGroup.Elements("object"))
            {
                Objects.Add(new TmxObject(element));
            }
    ...
    public class TmxObject : ITmxElement
    {
        ...
        public TmxObject(XElement xObject)
        {
            ...
            if (xGid != null)
            {
                Tile = new TmxLayerTile((uint)xGid, Convert.ToInt32(Math.Round(X)), Convert.ToInt32(Math.Round(X)));
        ...
        public Collection<TmxObjectPoint> ParsePoints(XElement xPoints)
        {
            ...
            {
                TmxObjectPoint pt = new TmxObjectPoint(s);
                points.Add(pt);
            }
            return points;
        }
    ...
    public class TmxObjectPoint
    {
        ...
        {
            var pt = s.Split(',');
            X = double.Parse(pt[0], NumberStyles.Float, CultureInfo.InvariantCulture);
            Y = double.Parse(pt[1], NumberStyles.Float, CultureInfo.InvariantCulture);
        }
    }
