...
public class TmxMap : TmxDocument
{
    ...
    private void Load(XDocument xDoc)
    {
        ...
        if (xMap == null) throw new Exception("Map file does not contain a 'map' element.");
        ...
        Tilesets = [];
        foreach (var element in xMap.Elements("tileset"))
        {
        ...
        foreach (var element in xMap.Elements().Where(x => x.Name == "layer" || x.Name == "objectgroup" || x.Name == "imagelayer" || x.Name == "group"))
        {
