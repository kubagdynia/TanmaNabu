...
public class TileMap : ITileMap
{
        ...
        bool verticalFlip = false, bool horizontalFlip = false, bool diagonalFlip = false)
    {
        ...
        fixed (Vertex* fptr = vertices)
        {
            ...
            Vector2f[] textCoords =
            [
                new(GetWorldTileSize.X * x, GetWorldTileSize.Y * y),
                new(GetWorldTileSize.X * x + GetWorldTileSize.X, GetWorldTileSize.Y * y),
                new(GetWorldTileSize.X * x + GetWorldTileSize.X, GetWorldTileSize.Y * y + GetWorldTileSize.Y),
                new(GetWorldTileSize.X * x, GetWorldTileSize.X * y + GetWorldTileSize.Y)
            ];
    ...
    private Vector2f GetViewSize(View view)
        => view.Size / _mapData.TileWorldDimension;

    private Vector2f GetViewCenter(View view)
        => view.Center / _mapData.TileWorldDimension;
