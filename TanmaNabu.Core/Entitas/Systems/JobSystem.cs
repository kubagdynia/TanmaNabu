...
namespace Entitas
{
    ...
    public abstract class JobSystem<TEntity> : IExecuteSystem where TEntity : class, IEntity
    {
        ...
        public void Execute()
        {
            int threadsRunning = _threads;
            TEntity[] entities = _group.GetEntities();
            int remainder = entities.Length % _threads;
            int slice = entities.Length / _threads + (remainder == 0 ? 0 : 1);
            ...
            foreach (Job<TEntity> job in _jobs)
            {
