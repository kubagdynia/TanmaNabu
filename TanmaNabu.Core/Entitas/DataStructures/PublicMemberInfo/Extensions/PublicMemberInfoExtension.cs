...
namespace Entitas
{
    public static class PublicMemberInfoExtension
    {
        public static List<PublicMemberInfo> GetPublicMemberInfos(this Type type)
        {
            ...
            List<PublicMemberInfo> memberInfos = new List<PublicMemberInfo>(fieldInfos.Length + propertyInfos.Length);
        ...
        {
            List<PublicMemberInfo> memberInfos = source.GetType().GetPublicMemberInfos();
            for (int i = 0; i < memberInfos.Count; i++)
            {
                PublicMemberInfo info = memberInfos[i];
